...
function parseMp4DurationSeconds(buf: Uint8Array): number {
  try {
...
// and mvhd-reported duration > 0 seconds.
function validateMp4Buffer(buf: ArrayBuffer, contentType: string | null, sceneNumber: number): number {
  ...
  console.log(`Scene ${sceneNumber} MP4 validated: ${(buf.byteLength / 1024).toFixed(0)}KB, duration ${durationSec.toFixed(2)}s, mime ${contentType || 'n/a'}`)
  return durationSec
}
...
async function uploadVideoToStorage(
  ...
  try {
    ...
    // STRICT validation: MIME video/*, MP4 ftyp signature, duration > 0
    const durationSec = validateMp4Buffer(videoBuffer, contentType, sceneNumber)
    ...
    if (!data.publicUrl || !data.publicUrl.endsWith('.mp4')) {
      throw new Error(`Stored scene ${sceneNumber} did not produce a valid .mp4 URL`)
    }
    console.log(`Scene merged: scene ${sceneNumber} MP4 stored (${durationSec.toFixed(2)}s) at ${data.publicUrl.substring(0, 120)}`)
    return { url: data.publicUrl, durationSec }
  } catch (err) {
